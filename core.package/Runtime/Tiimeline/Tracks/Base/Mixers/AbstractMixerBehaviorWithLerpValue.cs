...
namespace NecroMacro.Core.Timeline
{
		...
		where TTrackBinding : Component
	{
		...
		protected abstract TBehaviorValue GetInitialValue(TTrackBinding trackBinding);
		...
		{
			InitialValue = GetInitialValue(trackBinding);
		...
		protected override void CorrectBehaviorAfterInitialization(TBehavior behavior)
		{
			...
			if (behavior.UseInitialValueAsStart) behavior.StartValue = InitialValue;
			if (behavior.UseInitialValueAsEnd) behavior.EndValue = InitialValue;
		}

		protected override void ProcessBehavior(TBehavior behavior, float normalizedTime, float weight)
		{
			float curveValue = behavior.EvaluateCurve(normalizedTime);
		...
		protected abstract TBehaviorValue GetChange(
			TBehavior behavior, TBehaviorValue startValue, TBehaviorValue endValue, float curveValue
		);

		protected abstract TBehaviorValue ModifyBlendedValue(
			TBehaviorValue current, TBehaviorValue change, float weight
		);
	}
}
