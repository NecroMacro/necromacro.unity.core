...
namespace NecroMacro.Core.Timeline
{
		...
		where TTrackBinding : Component
	{
		...
		{
			if (!(playerData is TTrackBinding trackBinding) || !trackBinding) return;
			...
			for (int i = 0; i < inputCount; i++)
			{
				...
				{
					//AnimationsDependencies.Logger.Error($"Can't create inputData: {exception.Message}");
					Debug.LogError($"Can't create inputData: {exception.Message}");
					continue;
				}

				if (!inputData.IsValid || !IsValidBehavior(inputData.Behavior)) continue;
				if (inputData.Behavior.WasJustEnded)
				{
					// TODO проверить, возможно будет выбираться не последний клип, а случайный
					if (wasJustEndedData != null) wasJustEndedData.Behavior.WasJustEnded = false;
		...
		protected virtual bool IsValidBehavior(TBehavior behavior)
		{
			return true;
		}
		...
		private class InputData
		{
			...
			{
				float inputTime = (float)_input.GetTime();
				if (inputTime < 0) return -1f;
				float inputDuration = (float)_input.GetDuration();
				if (inputDuration <= 0) return -1f;
