...
namespace NecroMacro.Core.Timeline
{
	...
	public class PlayableDirectorObserver : MonoBehaviour
	{
		...
		private void Start()
		{
			if (_director.playableGraph.IsValid() && _director.playableGraph.IsPlaying()) DirectorPlayed(_director);
		}
		...
		{
			if (director.extrapolationMode == DirectorWrapMode.Loop) return;
		...
		private IEnumerator WatchDirector()
		{
			...
			if (_director.state == PlayState.Paused) yield return null;
			...
			{
				CheckComplete();
				if (!_isCompleteCheckAvailable) yield break;
		...
		{
			if (!_isCompleteCheckAvailable || !IsAtEnd) return;
